...
static int64_t _BWDiskSpaceFree() {
    NSError *error = nil;
...
#pragma mark 为什么磁盘缓存使用的是信号量（dispatch_semaphore）？
/*
...
static void _BWDiskCacheInitGlobal() {
...
static BWDiskCache *_BWDiskCacheGetGlobal(NSString *path) {
    if (path.length == 0) return nil;
    _BWDiskCacheInitGlobal();
...
static void _BWDiskCacheSetGlobal(BWDiskCache *cache) {
...
# pragma mark 知识点1 dispatch_semaphore_t
/*
 1.可以作为锁使用
 2.异步并发线程使用
 */
...
- (void)_trimToFreeDiskSpace:(NSUInteger)targetFreeDiskSpace {
    ...
    int64_t diskFreeBytes = _BWDiskSpaceFree();
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"BWDiskCache init error" reason:@"BWDiskCache must be initialized with a path. Use 'initWithPath:' or 'initWithPath:inlineThreshold:' instead." userInfo:nil];
...
- (instancetype)initWithPath:(NSString *)path
    ...
    BWDiskCache *globalCache = _BWDiskCacheGetGlobal(path);
    if (globalCache) return globalCache;
    
    BWKVStorageType type;
    if (threshold == 0) {
        type = BWKVStorageTypeFile;
    } else if (threshold == NSUIntegerMax) {
        type = BWKVStorageTypeSQLite;
    } else {
        type = BWKVStorageTypeMixed;
    }
    
    BWKVStorage *kv = [[BWKVStorage alloc] initWithPath:path type:type];
    if (!kv) return nil;
...
- (id<NSCoding>)objectForKey:(NSString *)key {
    if (!key) return nil;
    Lock();
    BWKVStorageItem *item = [_kv getItemForKey:key];
    Unlock();
    ...
    if (object && item.extendedData) {
        [BWDiskCache setExtendedData:item.extendedData toObject:object];
    }
    return object;
}
...
- (void)setObject:(id<NSCoding>)object forKey:(NSString *)key {
    ...
    NSData *extendedData = [BWDiskCache getExtendedDataFromObject:object];
    NSData *value = nil;
    ...
    if (!value) return;
    NSString *filename = nil;
    if (_kv.type != BWKVStorageTypeSQLite) {
