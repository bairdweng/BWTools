...
@implementation BWKVStorageItem
@end

@implementation BWKVStorage {
...
- (BWKVStorageItem *)_dbGetItemFromStmt:(sqlite3_stmt *)stmt excludeInlineData:(BOOL)excludeInlineData {
    int i = 0;
    ...
    BWKVStorageItem *item = [BWKVStorageItem new];
...
- (BWKVStorageItem *)_dbGetItemWithKey:(NSString *)key excludeInlineData:(BOOL)excludeInlineData {
    NSString *sql = excludeInlineData ? @"select key, filename, size, modification_time, last_access_time, extended_data from manifest where key = ?1;" : @"select key, filename, size, inline_data, modification_time, last_access_time, extended_data from manifest where key = ?1;";
    sqlite3_stmt *stmt = [self _dbPrepareStmt:sql];
    if (!stmt) return nil;
    sqlite3_bind_text(stmt, 1, key.UTF8String, -1, NULL);
    
    BWKVStorageItem *item = nil;
    int result = sqlite3_step(stmt);
...
- (NSMutableArray *)_dbGetItemWithKeys:(NSArray *)keys excludeInlineData:(BOOL)excludeInlineData {
    ...
    do {
        result = sqlite3_step(stmt);
        if (result == SQLITE_ROW) {
            BWKVStorageItem *item = [self _dbGetItemFromStmt:stmt excludeInlineData:excludeInlineData];
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"BWKVStorage init error" reason:@"Please use the designated initializer and pass the 'path' and 'type'." userInfo:nil];
    return [self initWithPath:@"" type:BWKVStorageTypeFile];
}

- (instancetype)initWithPath:(NSString *)path type:(BWKVStorageType)type {
    if (path.length == 0 || path.length > kPathLengthMax) {
        NSLog(@"BWKVStorage init error: invalid path: [%@].", path);
        return nil;
    }
    if (type > BWKVStorageTypeMixed) {
        NSLog(@"BWKVStorage init error: invalid type: %lu.", (unsigned long)type);
        return nil;
    }
    ...
    if (![[NSFileManager defaultManager] createDirectoryAtPath:path
        ...
        ![[NSFileManager defaultManager] createDirectoryAtPath:[path stringByAppendingPathComponent:kTrashDirectoryName]
                                   withIntermediateDirectories:YES
                                                    attributes:nil
                                                         error:&error]) {
        NSLog(@"BWKVStorage init error:%@", error);
        return nil;
    }
    ...
    if (![self _dbOpen] || ![self _dbInitialize]) {
        ...
        if (![self _dbOpen] || ![self _dbInitialize]) {
            [self _dbClose];
            NSLog(@"BWKVStorage init error: fail to open sqlite db.");
            return nil;
        }
    }
...
- (BOOL)saveItemWithKey:(NSString *)key value:(NSData *)value filename:(NSString *)filename extendedData:(NSData *)extendedData {
    ...
    if (_type == BWKVStorageTypeFile && filename.length == 0) {
        return NO;
    }
    ...
    if (filename.length) {
        ...
        if (_type != BWKVStorageTypeSQLite) {
...
- (BOOL)removeItemForKey:(NSString *)key {
    ...
    switch (_type) {
        case BWKVStorageTypeSQLite: {
            return [self _dbDeleteItemWithKey:key];
        } break;
        case BWKVStorageTypeFile:
        case BWKVStorageTypeMixed: {
            NSString *filename = [self _dbGetFilenameWithKey:key];
            if (filename) {
                [self _fileDeleteWithName:filename];
            }
...
- (BOOL)removeItemsToFitSize:(int)maxSize {
    ...
    do {
        ...
        for (BWKVStorageItem *item in items) {
            if (total > maxSize) {
                if (item.filename) {
...
- (BWKVStorageItem *)getItemForKey:(NSString *)key {
    if (key.length == 0) return nil;
    BWKVStorageItem *item = [self _dbGetItemWithKey:key excludeInlineData:NO];
    if (item) {
...
- (BWKVStorageItem *)getItemInfoForKey:(NSString *)key {
    if (key.length == 0) return nil;
    BWKVStorageItem *item = [self _dbGetItemWithKey:key excludeInlineData:YES];
    return item;
}
